...
func is_key_event_valid(
	...
	if not kEvent.pressed or not kEvent.echo: return false;
...
func log_connections(connections: Array[int]) -> bool:
	...
	for connection_err:int in connections:
		...
		error_index += 1;
	var caller_inf := get_stack()[1] as Dictionary;
	var caller_file := str(caller_inf.get('source', '?'));
	var caller_func := str(caller_inf.get('function', '?'));
	var caller_line := int(str(caller_inf.get('line', -1)));
...
func dir_assert(dir_path: StringName) -> Error:
	var exists := DirAccess.dir_exists_absolute(dir_path);
	if exists: return OK;
	var make_err: Error = DirAccess.make_dir_recursive_absolute(dir_path);
	if not make_err: return OK;
	print_debug("Failed to assert directory '%s'! (Error #%s)"%[
		dir_path, make_err
	]);
	return make_err;
