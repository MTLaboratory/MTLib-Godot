extends Node
## Modern Tool Library for Godot 4.2.2

const VERSION:StringName = &"0.3.2"; # MAJ.MIN.REV
...
func is_key_event_valid(
	...
	if kEvent.echo or not(kEvent.pressed): return false;
	if not(expectingKey == -1):
		if (kEvent.keycode != expectingKey): return false;
	return true;

func obj_encode(obj:Object) -> EncodedObjectAsID:
	var encode:EncodedObjectAsID = EncodedObjectAsID.new();
	encode.object_id = obj.get_instance_id();
	return encode;
func obj_decode(id:EncodedObjectAsID) -> Object:
	return instance_from_id(id.object_id);

## Loops through and array of Nodes, and calls queue_free on them.
func nodes_queue_free(node_array:Array[Node]) -> void:
...
func log_connections(connections:Array[int]) -> bool:
	...
	return false;
func var_null_or_invalid(val:Variant) -> bool:
	if (val == null): return true;
	if not is_instance_valid(val): return true;
	return false;
func var_valid(val:Variant) -> bool:
	return not var_null_or_invalid(val);
func node_valid_and_ready(node:Node) -> bool:
...
func var_get_sha256(val:Variant) -> String:
	return str(val).sha256_text();
## Returns a constant identifier for a given Node,
## based upon the Node's path in the SceneTree.
func node_get_id(node:Node) -> StringName:
...
func node_free_children(node: Node) -> void:
	if var_null_or_invalid(node): return;
	for child:Node in node.get_children():
		if var_null_or_invalid(child): continue;
		child.queue_free();
	return;
## Conditionally builds a string based upon the arguments based upon which
## are possibly already contained within one another.
func cond_string_build(strings: Array[String]) -> String:
	var final_str:String = '';
	for string:String in strings:
		if (not final_str.contains(string) and not string.contains(final_str)):
			final_str += string;
	return final_str;
### Returns the nearest Dict element to i, where i is an int.
### Beware, this does not work with dicts where the keys are not ints.
#func dict_get_nearest_to_key(dict:Dictionary, key:int) -> Variant:
	...
	#for dict_key:Variant in dict.keys():
		...
		#if (last_key >= dict_key_int): continue;
		#pass
		#if ((nearest_key == -1) or\
			#(absi(key - dict_key_int) < absi(key - nearest_key))):
			#nearest_key = dict_key_int;
		#last_key = dict_key_int;
	#return dict.get(nearest_key);
#func dict_get_nearest_to_val(dict:Dictionary, value:Variant) -> Variant:
	#if dict.is_empty(): return null;
	#pass
	#var _nearest_key:int = -1;
	#var nearest_value:Variant = null;
	#var min_difference:float = INF;
	#for dict_key:Variant in dict.keys():
		#var current_value:Variant = dict[dict_key];
		#var difference:float = absf(value - current_value);
		#if not(difference < min_difference): continue;
		#pass
		#min_difference = difference;
		#_nearest_key = dict_key;
		#nearest_value = current_value;
	#return nearest_value;
## Makes a [Color] fully opaque. (Alpha 1)
func color_make_opaque(color: Color) -> Color:
	return Color(color.r, color.g, color.b, 1.);
## Returns a vector that is useful for aiming from one moving
## RigidBody2D, at another.
func vector_foretell(
	from:RigidBody2D,
	to:RigidBody2D,
) -> Vector2:
	...
	var q_root:float =\
		sqrt(2* dist * sT * cos_theta +4*(sB * sB - sT * sT) * dist * dist);
...
func moving_towards(
	...
	angle_tolerance_degrees:float = 1.,
) -> bool:
	var direction_to_target:Vector2 =\
		(target_body.global_position - source_body.global_position).normalized();
...
func dir_assert(dir_path:StringName) -> void:
...
func dir_get_resources(dir_path:StringName) -> Array[Resource]:
	...
	for file:String in DirAccess.get_files_at(dir_path):
		if not(file.ends_with('.tres') or file.ends_with('.remap')):
			continue;
		var f_path:String = str(dir_path + file);
		## (godot/issues/76823 & (issues/66014 initial)) ～(　TロT)σ
		if f_path.ends_with('.remap'): f_path = f_path.trim_suffix('.remap');
		var res:Resource = load(f_path);
		if MTLib.var_valid(res): dirResources.append(res);
		else: print_debug("Failed to load Resource '%s'!"%f_path)
	if dirResources.is_empty():
		print_debug("Folder '%s' (apparently) has no Resources"%dir_path)
	else:
		#print_debug("Got (%s) Resources in '%s'."%\
		#	[dirResources.size(), dir_path])
		pass
	return dirResources;
## @deprecated
class Random:
